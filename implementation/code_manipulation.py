...
class ProgramVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(
        self, node: ast.FunctionDef  # pylint: disable=invalid-name
    ) -> None:
...
def _yield_token_and_is_call(code: str) -> Iterator[tuple[tokenize.TokenInfo, bool]]:
    ...
    try:
        ...
        for token in tokens:
            if (
                prev_token
                and prev_token.type == tokenize.NAME  # If the previous token exists and
                and token.type == tokenize.OP  # it is a Python identifier
                and token.string == "("  # and the current token is a delimiter
